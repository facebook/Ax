...
Each metric is responsible for fetching its own data. Thus, all metric classes must implement the method `fetch_trial_data`, which accepts a [`Trial`](https://ax.readthedocs.io/en/0.5.0/core.html#ax.core.trial.Trial) and returns an instance of [`Data`](https://ax.readthedocs.io/en/0.5.0/core.html#ax.core.data.Data), a wrapper around a Pandas DataFrame.

To fetch data for an experiment or trial, use `exp.fetch_data` or `trial.fetch_data`. These methods fetch data for all metrics on the experiment and then combine the results into a new aggregate [`Data`](https://ax.readthedocs.io/en/0.5.0/core.html#ax.core.data.Data) instance.
