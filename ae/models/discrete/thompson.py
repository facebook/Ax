...
class ThompsonSampler(DiscreteModel):
    ...
    Conditions are given weight proportional to the probability that they are
    ...
    def __init__(
        ...
        Args:
            ...
            min_weight (float, Optional): The minimum weight a condition must be
                given in order for it to be returned from the gernerator. If not
                specified, will be set to 2 / (number of conditions).
            uniform_weights (bool): If True, the conditions returned from the
                generator will each be given a weight of 1 / (number of conditions).
        """
    ...
    def gen(
        ...
        weighted_conditions = [
            ...
            (weights[i], np.random.random(), conditions[i])
            for i in range(k)
            if weights[i] > min_weight
        ]

        if len(weighted_conditions) == 0:
            raise ValueError(
                "No arms generated by Thompson Sampling had weight > min_weight. "
                + f"The minimum weight required is {min_weight:2.4}, "
                + "and the maximum weight of any arm generated "
                + f"is {max(weights):2.4}."
            )

        weighted_conditions.sort(reverse=True)
        top_weighted_conditions = (
            weighted_conditions[:n] if n > 0 else weighted_conditions
        )
        top_conditions = [condition for _, _, condition in top_weighted_conditions]
        top_weights = [weight for weight, _, _ in top_weighted_conditions]

        if self.uniform_weights:
            top_weights = [1 / len(top_conditions) for _ in top_conditions]

        return top_conditions, [x / sum(top_weights) for x in top_weights]
    ...
    def _generate_weights(
        ...
        if fraction_all_infeasible > 0.99:
            raise ValueError(
                "Less than 1% of samples have a feasible condition. "
    ...
    def _produce_samples(
        ...
        best_condition = objective_values.max(axis=0)  # (num_samples,)
        all_conditions_infeasible = best_condition == -np.Inf  # (num_samples,)
        fraction_all_infeasible = all_conditions_infeasible.mean()
        filtered_objective = objective_values[:, ~all_conditions_infeasible]  # (k x ?)
        return filtered_objective, fraction_all_infeasible

    def _validate_Xs(self, Xs: List[List[TParamValueList]]) -> None:
        """
        1. Require that all Xs have the same conditions, i.e. we have observed
        all conditions for all metrics. If so, we can safely use Xs[0] exclusively.
        ...
        if not all(x == Xs[0] for x in Xs[1:]):
            raise ValueError(
                ...
                "i.e. that we have observed all conditions for all metrics."
            )
