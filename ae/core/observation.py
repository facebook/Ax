...
class ObservationFeatures(Base):
    ...
    These include both the condition parameters and the features of the
    ...
    row of Data joined with the condition parameters.
    ...
    Attributes:
        parameters (TParameterization): condition parameters
    ...
    def from_condition(
        ...
        """Convert a Condition to an ObservationFeatures, including additional
        data as specified.
        """
...
class Observation(Base):
    ...
    (ObservationData). Optionally, a condition name associated with the
    features.
...
def observations_from_data(experiment: Experiment, data: Data) -> List[Observation]:
    ...
    condition parameters from experiment.

    Uses a diagonal covariance matrix across metric_names.

    Args:
        experiment: Experiment with condition parameters.
    ...
    for g, d in data.df.groupby(by=feature_cols):
        ...
        obs_kwargs = {}
        obs_kwargs["parameters"] = experiment.conditions_by_name[
            features["condition_name"]
        ].params.copy()
