...
from ae.lazarus.ae.generator.cross_validation import (
    CVResult,
    compute_diagnostics,
    cross_validate,
)
...
class CrossValidationTest(TestCase):
    ...
    def testCrossValidate(self):
        ...
        ma.get_training_data = mock.MagicMock(
            "ae.lazarus.ae.generator.base.Generator.get_training_data",
            autospec=True,
            return_value=self.training_data,
        )
        ma.cross_validate = mock.MagicMock(
            "ae.lazarus.ae.generator.base.Generator.cross_validate",
            autospec=True,
            return_value=self.observation_data,
        )
        # Do cross validation
        with self.assertRaises(ValueError):
            cross_validate(generator=ma, folds=4)
        with self.assertRaises(ValueError):
            cross_validate(generator=ma, folds=0)
        # First 2-fold
        result = cross_validate(generator=ma, folds=2)
        self.assertEqual(len(result), 4)
        # Check that Generator.cross_validate was called correctly.
        ...
        # Test LOO
        result = cross_validate(generator=ma, folds=-1)
        self.assertEqual(len(result), 4)
        ...
        result = cross_validate(generator=ma, folds=-1, test_selector=test_selector)
