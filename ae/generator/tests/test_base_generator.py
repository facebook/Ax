...
class BaseGeneratorTest(TestCase):
    ...
    @mock.patch(
        "ae.lazarus.ae.generator.base.unwrap_observation_data",
        autospec=True,
        return_value=(2, 2),
    )
    @mock.patch(
    ...
    def testGenerator(
        ...
        transforms = [t1, t2]
        exp = get_experiment()
        generator = Generator(search_space_for_value(), 0, transforms, exp, 0)
        ...
        generator._gen.assert_called_with(
            n=1,
        ...
        generator._cross_validate = mock.MagicMock(
            "ae.lazarus.ae.generator.base.Generator._cross_validate",
            autospec=True,
            return_value=[observation1trans().data],
        )
        cv_training_data = [observation2()]
        cv_test_points = [observation1().features]
        cv_predictions = generator.cross_validate(
            cv_training_data=cv_training_data, cv_test_points=cv_test_points
        )
        generator._cross_validate.assert_called_with(
        ...
        obs = generator.get_training_data()
        self.assertTrue(obs == [observation1(), observation2()])
        self.assertEqual(generator.metric_names, {"a", "b"})
        self.assertIsNone(generator.status_quo)
        self.assertTrue(generator.model_space == search_space_for_value())
        self.assertEqual(generator.training_in_design, [True, True])

        generator.training_in_design = [True, False]
        with self.assertRaises(ValueError):
            generator.training_in_design = [True, True, False]
    ...
    @mock.patch(
        "ae.lazarus.ae.generator.base.Generator.predict",
        autospec=True,
        return_value=None,
    )
    def testGenWithDefaults(self, _, mock_gen):
        ...
        generator = Generator(ss, None, [], exp)
        generator.gen(1)
