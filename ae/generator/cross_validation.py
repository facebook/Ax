...
import numpy as np
from ae.lazarus.ae.core.observation import Observation, ObservationData
...
def cross_validate(
    generator: Generator, folds: int = -1, test_selector: Optional[Callable] = None
) -> List[CVResult]:
    """Cross validation for generator predictions.
    ...
    Args:
        generator: Fitted generator to cross validate.
    ...
    for train_names, test_names in _gen_train_test_split(
        ...
        if len(cv_test_points) == 0:
            continue
        # Make the prediction
        cv_test_predictions = generator.cross_validate(
