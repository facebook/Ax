---
id: experiment
title: Internal Organization of Ax -- Experiment
---

import ask_tell_simple from './assets/ask_tell_simple.png';
import ask_tell_flowchart from './assets/ask_tell_flowchart.svg';
import experiment_composition from './assets/experiment_composition.svg';
import trial_composition from './assets/trial_composition.svg';
import search_space_composition from './assets/search_space_composition.svg';
import optimization_config_composition from './assets/optimization_config_composition.svg';
...
# Internal Organization of Ax -- Experiment

As we discuss in Intro to [Adaptive Experimentation](./intro-to-ae), every
optimization in Ax is an iterative process where we:

- Generate candidate datapoints to evaluate (represented by Trials)
...
- **A collection of indexed `Trial`-s** (or `BatchTrial`-s, more on these
  below), each of which contain one or more `Arm`-s (representing a point that
...
- "What Ax should optimize for" (via `OptimizationConfig`, composed of one or
  multiple `Objective`-s and `OutcomeConstraints`),
...
## Further Reading
Learn about Ax `GenerationStrategy` and Orchestration on the following pages:
- [Internal Organization of Ax -- Orchestration](./orchestration)
