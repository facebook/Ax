...
1. Define the metric(s) You Want to Track
2. Call `configure_metrics` to add the metrics to your experiment

### 1. Define the metric(s) You Want to Track
Define and construct the metrics you want to track. Ensure that the constructed metric(s) are subclasses of Ax's `IMetric`.
...
custom_metric1 = CustomMetric(name="custom_metric1")
custom_metric2 = CustomMetric(name="custom_metric2")
```

### 2. Call `configure_metrics` to add the metrics to your experiment
Call the `add_tracking_metrics` method, passing in the list of metrics we defined previously.
...
client.configure_metrics([custom_metric1, custom_metric2])
```
