...
* **Unique metric names**: Ensure that each metric has a unique name. If a metric with the same name already exists on the experiment, an error will be raised and none of the metrics will be added to the experiment.
* **Optimization config conflicts**: If a metric is already present in the experiment's optimization config, an error will be raised. Remove the metric from the optimization config before adding it as a tracking metric.
