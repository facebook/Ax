...
Ax provides a convenient way to save and load experiments in SQLite format, making it easy to store and manage experiment data. In this recipe, we will walk through the steps of saving and loading an Ax experiment to/from SQLite.

## Introduction

Saving and loading an experiment to/from SQLite is useful when you want to store the experiment data in a structured and queryable format. This can be particularly useful for production applications that require a centralized, high-performance database.
...
In this recipe, we plan to save a a snapshot of an Ax Client to SQLite, reload the client from the SQLite instance, and validate its contents.
...
Instantiate a `Client` with a StorageConfig and configure it for your experiment. In order to initialize a StorageConfig, we need a URL pointing to your SQLite database. Such a URL is typically composed of a dialect (e.g. sqlite, mysql, postgresql), optional driver (DBAPI used to connect to the database; e.g. psycopg2 for postgresql), username, password, hostname, and database name.
...
### Load an Experiment from SQLite
#### 1. Call load_from_database to initialize a new Client
...
If you have custom metrics or runners that you want to ensure are saved to SQLite properly, you can initialize the `Client` with a `StorageConfig` that contains a `RegistryBundle`, that bundles together encoding and decoding logic for use in the save/load functions.
