...
def get_current_lilo_hash(experiment: Experiment) -> str | None:
    ...
    Looks up the pairwise preference ``DerivedMetric`` on the experiment,
    extracts its ``input_metric_names``, and computes the hash.

    Returns:
        The SHA-256 hex digest of the current LILO input state, or ``None``
        if no pairwise ``DerivedMetric`` is registered on the experiment.
    """
