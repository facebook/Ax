...
Experiments in the 'Standard' tier use standard features that are \
...
def _check_if_is_in_standard_other_settings(
    ...
    why_not_is_in_standard: list[str],
    why_not_supported: list[str],
    not_standard_api_message: str = NOT_STANDARD_API_MESSAGE,
) -> tuple[bool, bool]:
...
def format_tier_message(
    ...
    if tier == "Standard":
        ...
        if why_not_is_in_standard:
            ...
            why_msg = (
                "\n\nWhy this experiment is not in the 'Standard' tier: "
                f"\n{why_msg}\n"
            )
            msg += why_msg
