...
class TorchAdapter(Adapter):
    ...
    def _convert_observations(  # TODO rename?
        self,
        ...
        Returns:
            ...
            - An optional list of lists of candidate metadata. Each inner list
                ...
                to one observation.
            # TODO: What do we do with candidate metadata? Is it needed?
            # IT is used by TRBO, so we probably need to keep it.
        """
        if len(outcomes) == 0:
            return [], [], None
        ...
        # Join mean & arms to repeat the arm rows if necessary.
        ...
        for outcome in outcomes:
            if outcome not in mean_and_params:
                raise DataRequiredError(
                    f"Attempting to extract a dataset for {outcome=} but no data "
            ...
            Yvar = sems_df[outcome].to_numpy()[to_keep]
            if np.all(np.isnan(Yvar)):
                Yvar = None
            else:
                Yvar = torch.from_numpy(Yvar).double().square().view(-1, 1)
