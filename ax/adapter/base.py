...
class Adapter:
    ...
    def _set_and_filter_training_data(
        ...
        include only in-design points if ``_fit_out_of_design=True``.
        """
        ...
        if self._data_loader_config.fit_out_of_design:
            self._training_in_design = [True] * len(experiment_data.arm_data)
        else:
            self._training_in_design = self._compute_in_design(
    ...
    def _compute_in_design(
        ...
        # TODO: We should implement a more efficient membership check here.
        # Might be tricky to implement for HSS.
        return [
    ...
    def _set_status_quo(self, experiment: Experiment) -> None:
        ...
        if target_trial_index is None:
            ...
            return
        # Filter the dataframe to find trial, arm combination that matches the SQ.
        obs_data = self._training_data.observation_data
        metric_names = self._training_data.metric_names

        status_quo_observations = []
        try:
            sq_obs_data = obs_data.loc[(target_trial_index, self.status_quo_name)]
            map_key = sq_obs_data.index.name
            for map_value, row in (
                sq_obs_data.iterrows()
                if isinstance(sq_obs_data, DataFrame)
                else [(None, sq_obs_data)]  # No map key column in obs data.
            ):
                mean, sem = row["mean"], row["sem"]
                metric_names = mean.dropna().index.tolist()
                status_quo_observations.append(
                    Observation(
                        features=ObservationFeatures(
                            parameters=status_quo_arm.parameters,
                            trial_index=target_trial_index,
                            metadata={map_key: map_value}
                            if map_value is not None
                            else None,
                        ),
                        data=ObservationData(
                            metric_names=metric_names,
                            means=mean[metric_names].to_numpy(),
                            covariance=np.diag(sem[metric_names].to_numpy() ** 2),
                        ),
                        arm_name=self._status_quo_name,
                    )
                )
        except KeyError:
            # No SQ data found. Warning is raised in next if block.
            pass
    ...
    def status_quo_data_by_trial(self) -> dict[int, ObservationData] | None:
        ...
        """
        # TODO: Could possibly extract from the experiment directly. See D72685267.
    ...
    def get_training_data(self, filter_in_design: bool = False) -> ExperimentData:
        ...
        Args:
            filter_in_design: If True, the data is filtered by ``training_in_design``.
                Note that this will include all points if ``fit_out_of_design is True``.
        """
