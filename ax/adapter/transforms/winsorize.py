...
class Winsorize(Transform):
    ...
    def transform_experiment_data(
        ...
        lower_bounds = {m: c[0] for m, c in self.cutoffs.items()}
        upper_bounds = {m: c[1] for m, c in self.cutoffs.items()}
        obs_data["mean"] = obs_data["mean"].clip(lower=lower_bounds, upper=upper_bounds)
