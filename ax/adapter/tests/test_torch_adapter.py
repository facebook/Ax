...
class TorchAdapterTest(TestCase):
    ...
    def test_get_transformed_model_gen_args_with_target_point(self) -> None:
        ...
        experiment = get_branin_experiment(with_completed_trial=True)
        target_arm = Arm(parameters={"x1": -5.0, "x2": 15.0})
        ...
        # Assert: confirm target_point is correctly extracted and transformed
        self.assertIsNotNone(torch_opt_config.target_point)
        expected_target = torch.tensor([0.0, 1.0], dtype=torch.double)
        torch.testing.assert_close(torch_opt_config.target_point, expected_target)

    def test_get_transformed_model_gen_args_no_target_point(self) -> None:
        # Test that _get_transformed_model_gen_args handles None target_arm correctly
        ...
        # Assert: confirm target_point is None when no target_arm is provided
        self.assertIsNone(torch_opt_config.target_point)
