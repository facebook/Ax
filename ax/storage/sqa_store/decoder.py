...
class Decoder:
    ...
    def _auxiliary_experiments_by_purpose_from_experiment_sqa(
        ...
        if experiment_sqa.auxiliary_experiments:
            from ax.storage.sqa_store.load import _get_experiment_name, load_experiment
            ...
            for auxiliary_experiment_sqa in experiment_sqa.auxiliary_experiments:
                ...
                # load it again.
                if any(
                    auxiliary_experiment_sqa.target_experiment_id
                    == aux_exp.experiment.db_id
                    for aux_exp in auxiliary_experiments_by_purpose[purpose]
                ):
                    continue
                target_experiment_name = none_throws(
                    _get_experiment_name(
                        experiment_id=auxiliary_experiment_sqa.target_experiment_id,
                        config=self.config,
                    )
                )
