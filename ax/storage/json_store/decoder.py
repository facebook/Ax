...
def object_from_json(
    ...
    if type(object_json) in (str, int, float, bool, type(None)) or isinstance(
        ...
        elif isclass(_class) and issubclass(_class, GenerationBlockingCriterion):
            # GenerationBlockingCriterion is similar to TransitionCriterion
            return generation_blocking_criterion_from_json(
                blocking_criterion_class=_class,
                object_json=object_json,
                **vars(registry_kwargs),
            )
...
def generation_blocking_criterion_from_json(
    blocking_criterion_class: type[GenerationBlockingCriterion],
    object_json: dict[str, Any],
    decoder_registry: TDecoderRegistry = CORE_DECODER_REGISTRY,
    class_decoder_registry: TClassDecoderRegistry = CORE_CLASS_DECODER_REGISTRY,
) -> GenerationBlockingCriterion:
    """Load GenerationBlockingCriterion from JSON."""
    return _criterion_from_json(
        criterion_class=blocking_criterion_class,
        object_json=object_json,
        decoder_registry=decoder_registry,
        class_decoder_registry=class_decoder_registry,
    )
...
def generation_node_from_json(
    ...
    if transition_criteria_json is not None:
        ...
        for tc_json in transition_criteria_json:
            ...
            if tc_json.get("block_gen_if_met") is True:
                if tc_type == "MaxGenerationParallelism":
                    migrated_blocking_criteria.append(tc_json)
                elif tc_type == "MinTrials":
                    # Copy and update type
                    blocking_json = tc_json.copy()
                    blocking_json["__type"] = "MaxTrialsAwaitingData"
                    migrated_blocking_criteria.append(blocking_json)
        ...
        # Merge migrated criteria with any existing generation_blocking_criteria
        if generation_blocking_criteria_json is None:
            generation_blocking_criteria_json = migrated_blocking_criteria
        else:
            generation_blocking_criteria_json = (
                migrated_blocking_criteria + generation_blocking_criteria_json
            )
    ...
    return GenerationNode(
        ...
        generation_blocking_criteria=object_from_json(
            object_json=generation_blocking_criteria_json,
            decoder_registry=decoder_registry,
            class_decoder_registry=class_decoder_registry,
        ),
