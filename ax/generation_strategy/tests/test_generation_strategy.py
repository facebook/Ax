...
class TestGenerationStrategyWithoutAdapterMocks(TestCase):
    ...
    def test_with_model_selection(self, mock_model_state: Mock) -> None:
        ...
        best_model_idx = 0
        best_model_selector.best_model.side_effect = (
            lambda generator_specs: generator_specs[best_model_idx]
        )
    ...
    def test_gen_with_parameter_constraints(self) -> None:
        ...
        Note: Order constraints on ChoiceParameters work because ordering is
        preserved after rounding to discrete values. Sum constraints (e.g.,
        x + y <= 6) may not be enforced correctly due to the constraint
        checking happening before rounding in the generation pipeline.
        """
        ...
        # Generate multiple trials and verify constraints are satisfied
        for i in range(10):  # 10 possible parameterizations
            ...
            self.assertLessEqual(
                x1_val,
                x2_val,
                f"Constraint x1 <= x2 violated in trial {i}: {x1_val} > {x2_val}",
            )

            # Attach the trial to experiment for next iteration
...
class TestGenerationStrategy(TestCase):
    ...
    def test_sobol_MBM_strategy_batches(self) -> None:
        ...
        grs = sobol_MBM_generation_strategy.gen(exp, n=2)[0]
        exp.new_batch_trial(generator_runs=grs).run()
        for i in range(1, 8):
            if i == 7:
                # Check completeness error message.
                with self.assertRaises(GenerationStrategyCompleted):
                    grs_2 = sobol_MBM_generation_strategy.gen(exp, n=2)[0]
            else:
                grs_2 = sobol_MBM_generation_strategy.gen(exp, n=2)[0]
    ...
    def test_gen(self) -> None:
        ...
        self.assertEqual(generator_spec_gen_mock.call_count, 3)
        pending_in_each_gen = enumerate(
            args_and_kwargs.kwargs.get("pending_observations")
            for args_and_kwargs in generator_spec_gen_mock.call_args_list
        )
        ...
        # gen are present in the pending points
        for gr, (idx, pending) in zip(grs, pending_in_each_gen):
            exp.new_trial(generator_run=gr[0]).mark_running(no_runner_required=True)
            if idx > 0:
                prev_grs = grs[idx - 1]
                for arm in prev_grs[0].arms:
                    for m in pending:
                        self.assertIn(ObservationFeatures.from_arm(arm), pending[m])
        generator_spec_gen_mock.reset_mock()
    ...
    def test_gs_with_suggested_n_is_zero(self) -> None:
        ...
        for _i in range(3):
            ...
            grs = gs.gen(experiment=exp, n=5)[0]
        ...
        grs = gs.gen(experiment=exp, n=8)[0]
        self.assertEqual(len(grs), 2)
    ...
    def test_node_gs_with_auto_transitions(self) -> None:
        ...
        trial0 = exp.new_batch_trial(generator_runs=gs.gen(exp)[0])
        self.assertEqual(gs.current_node_name, "sobol")
        ...
        trial0.run()
        for _i in range(0, 2):
            trial = exp.new_batch_trial(generator_runs=gs.gen(exp)[0])
            self.assertEqual(gs.current_node_name, "sobol_3")
            self.assertEqual(len(trial.generator_runs), 3)
            self.assertEqual(trial.generator_runs[0]._generation_node_name, "mbm")
            self.assertEqual(trial.generator_runs[1]._generation_node_name, "sobol_2")
    ...
    def test_node_gs_with_auto_transitions_three_phase(self) -> None:
        ...
        trial0 = exp.new_batch_trial(generator_runs=gs_2.gen(exp)[0])
        self.assertEqual(gs_2.current_node_name, "sobol")
        trial0.run()
        ...
        # to the last first node in a trial.
        for _i in range(0, 2):
            trial = exp.new_batch_trial(generator_runs=gs_2.gen(exp)[0])
    ...
    def test_gs_with_fixed_features_constructor(self) -> None:
        ...
        trial0 = exp.new_batch_trial(generator_runs=gs.gen(exp, n=9)[0])
        ...
        with self.subTest("no passed fixed features gen"):
            with mock_patch_method_original(
                mock_path=f"{GeneratorSpec.__module__}.GeneratorSpec.gen",
                original_method=GeneratorSpec.gen,
            ) as generator_spec_gen_mock:
    ...
    def test_gs_with_input_constructor(self) -> None:
        ...
        for _i in range(0, 2):
            ...
            trial = exp.new_batch_trial(generator_runs=gs.gen(exp, n=9)[0])
            self.assertEqual(gs.current_node_name, "sobol_3")
