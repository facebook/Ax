...
def filter_trials_by_status(
    ...
    if only_in_statuses is not None:
        trials_to_check = get_trials_by_status(
...
# ============================================================================
# GenerationBlockingCriterion - for blocking generation without transitioning
# ============================================================================


class GenerationBlockingCriterion(SortableBase):
    """A criterion that blocks generation from a GenerationNode without triggering
    ...
    def criterion_class(self) -> str:
        """Name of the class of this GenerationBlockingCriterion."""
    ...
    def _unique_id(self) -> str:
        """Unique id for this GenerationBlockingCriterion."""
        return str(self)


class TrialCountBlockingCriterion(GenerationBlockingCriterion):
    """Abstract base class for blocking criteria based on trial count thresholds.

    This class provides shared logic for blocking criteria that count trials toward
    a threshold. Subclasses only need to implement `block_continued_generation_error()`
    to define the specific error raised when generation is blocked.
...
class MaxGenerationParallelism(TrialCountBlockingCriterion):
    """A GenerationBlockingCriterion that blocks generation after a maximum number
    of trials have been generated for the current GenerationNode and are currently
    running.
...
class MaxTrialsAwaitingData(TrialCountBlockingCriterion):
    """A GenerationBlockingCriterion that blocks generation after a maximum number
    of trials have been generated, waiting for data before allowing more generation.

    This criterion blocks generation from the associated GenerationNode when the
    ...
    Args:
        ...
        use_all_trials_in_exp: A flag to use all trials in the experiment, instead of
            only those generated by the current GenerationNode. Defaults to True.
        count_only_trials_with_data: If set to True, only trials with data will be
            counted towards the ``threshold``. Defaults to False.
    """
...
class TrialBasedCriterion(TransitionCriterion):
    ...
    def experiment_trials_by_status(
        ...
        return get_trials_by_status(experiment=experiment, statuses=statuses)
