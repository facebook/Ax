...
class GenerationStrategy(GenerationStrategyInterface):
    ...
    def num_baseline_trials(self) -> int:
        ...
        return self._steps[0].num_trials
    ...
    def gen_multiple_with_ensembling(
        ...
        """Produce Generator runs for multiple trials at once with the possibility
        of ensembling, or using multiple models per trial, getting multiple
        GenerateRuns per trial.
        ...
        Returns:
            A list of lists generator runs. Each outer list represents a trial
            being suggested and  each inner list represents a generator run for that
            trial.
        """
        ...
        return [
            [gr]
            for gr in self._gen_multiple(
                experiment=experiment,
                num_generator_runs=num_generator_runs,
                data=data,
                n=n,
                pending_observations=pending_observations,
                extra_gen_metadata=extra_gen_metadata,
            )
        ]
    ...
    @copy_doc(GenerationStrategyInterface.get_hypervolume)
    def get_hypervolume(
        self,
        experiment: Experiment,
        ...
        current_model = self._curr.model
        ...
        model = get_model_from_generator_run(
            generator_run=not_none(self.last_generator_run),
            experiment=experiment,
    ...
    @copy_doc(GenerationStrategyInterface.get_pareto_optimal_parameters)
    def get_pareto_optimal_parameters(
        ...
        # Use existing modelbridge if it supports MOO otherwise create a new
        # MOO modelbridge to use for Pareto frontier extraction.
        ...
        if is_moo_modelbridge:
            self._fit_current_model(data=None)
        else:
        ...
        if not moo_optimization_config.objective_thresholds:
            ...
            logger.info(
                "Using inferred objective thresholds: "
                f"{objective_thresholds_override}, as objective thresholds "
                "were not specified as part of the optimization "
    ...
    @copy_doc(GenerationStrategyInterface.get_best_trial)
    def get_best_trial(
        self,
        experiment: Experiment,
        optimization_config: Optional[OptimizationConfig] = None,
