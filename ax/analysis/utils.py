...
def prepare_arm_data(
    ...
    Args:
        ...
        use_model_predictions: Whether to use an Adapter to predict the effects of each
            arm. This is often more trustoworthy (and leads to better reproducibility)
...
def _prepare_modeled_arm_data(
    ...
    predictions = adapter.predict(
        observation_features=[
            ObservationFeatures.from_arm(arm=arm) for _, arm in trial_index_arm_pairs
        ]
    )
...
def _prepare_raw_arm_data(
    ...
    # If trial_index is None, extract data from all trials.
        ...
        trial_index_arm_pairs = [
            (trial_index, arm) for arm in experiment.trials[trial_index].arms
        ]
        data_df = experiment.trials[trial_index].fetch_data().df
    else:
        trial_index_arm_pairs = [
            (trial.index, arm)
            for trial in experiment.trials.values()
...
def _extract_generation_node_name(trial: BaseTrial, arm: Arm) -> str:
    ...
    for gr in trial.generator_runs:
        if arm.signature in gr.arm_signatures:
            return gr._generation_node_name or "unknown"

    raise ValueError("Arm was not generated by any GenerationNode in the trial.")
