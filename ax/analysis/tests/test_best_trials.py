...
class TestBestTrials(TestCase):
    ...
    def test_compute_soo(self) -> None:
        ...
        self.assertIn("based on", card.subtitle.lower())
    ...
    def test_generation_strategy_requirements(self) -> None:
        ...
        with self.subTest(msg="GS not required for raw observations"):
            ...
            card = analysis.compute(
                experiment=client._experiment, generation_strategy=None
            )
    ...
    def test_trial_status_filter(self) -> None:
        ...
        # Manually change trial 2's status to non-completed
        client._experiment.trials[2]._status = TrialStatus.FAILED
    ...
    def test_use_model_predictions_with_raw_observations(self) -> None:
        """Test that use_model_predictions=False works with raw observations."""
        client = self.client
        ...
        # Execute: Compute BestTrials using raw observations
        analysis = BestTrials(use_model_predictions=False)
        card = analysis.compute(
            experiment=experiment, generation_strategy=generation_strategy
        )

        # Assert: Should return the best trial using raw observations
        self.assertEqual(card.name, "BestTrials")
        self.assertIsNotNone(card.df)
