...
_TRIAL_NUMBER_EXPLANATION = (
    "The x-axis represents trial number (1st completed trial, 2nd completed "
    "trial, etc.), not trial index. Trial indices can be non-sequential (e.g., "
    "0, 2, 5, 7 if some trials failed or were abandoned), but trial number counts "
    "only successfully completed trials in order."
)
...
class UtilityProgressionAnalysis(Analysis):
    ...
    following columns:
        - trial_number: Sequential trial completion number (1, 2, 3, ...)
    ...
    def validate_applicable_state(
        ...
        if none_throws(experiment).optimization_config is None:
    ...
    def compute(
        ...
        # Create DataFrame with 1-based trial numbers for user-friendly display
        # (1st trial, 2nd trial, etc. instead of 0-indexed)
        df = pd.DataFrame(
            {
                "trial_number": list(range(1, len(trace) + 1)),
                "utility": trace,
            }
        )
        ...
        if experiment.is_bope_problem:
            title = "Utility Progression: User Preference"
            y_label = "User Preference Score"
            subtitle = (
                "Shows the best user preference score achieved so far across "
                f"completed trials. {_TRIAL_NUMBER_EXPLANATION} "
                f"{_CUMULATIVE_BEST_EXPLANATION} "
                f"{_INFEASIBLE_TRIALS_EXPLANATION}"
            )
        elif experiment.is_moo_problem:
            title = "Utility Progression: Hypervolume"
            y_label = "Hypervolume"
            subtitle = (
                "Shows the hypervolume of the Pareto frontier achieved so far across "
                f"completed trials. {_TRIAL_NUMBER_EXPLANATION} The y-axis shows "
                "cumulative best hypervolume—only improvements are plotted, so flat "
                ...
                f"Pareto frontier relative to a reference point. "
                f"{_INFEASIBLE_TRIALS_EXPLANATION}"
            )
        else:
            ...
            if isinstance(objective, ScalarizedObjective):
                # Use the expression property from ScalarizedObjective
                expression = objective.expression
                title = f"Utility Progression: Scalarized Objective ({expression})"
                ...
                subtitle = (
                    ...
                    f"(objective is to {direction}). {_TRIAL_NUMBER_EXPLANATION} "
                    f"{_CUMULATIVE_BEST_EXPLANATION} "
                    f"{_INFEASIBLE_TRIALS_EXPLANATION}"
                )
            else:
                ...
                title = f"Utility Progression: Best {objective_name}"
                ...
                subtitle = (
                    ...
                    f"{_TRIAL_NUMBER_EXPLANATION} {_CUMULATIVE_BEST_EXPLANATION} "
                    f"{_INFEASIBLE_TRIALS_EXPLANATION}"
                )
        ...
        fig.update_xaxes(title_text="Trial Number", dtick=1)
