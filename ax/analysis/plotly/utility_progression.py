...
_ITERATION_NUMBER_EXPLANATION = (
    "The x-axis represents iteration number (1st completed trial, 2nd completed "
    "trial, etc.), not trial index. Trial indices can be non-sequential (e.g., "
    "0, 2, 5, 7 if some trials failed or were abandoned), but iteration number "
    "counts only successfully completed trials in order."
)
...
class UtilityProgressionAnalysis(Analysis):
    ...
    following columns:
        - iteration_number: Sequential trial completion number (1, 2, 3, ...)
    ...
    def compute(
        ...
        # Create DataFrame with 1-based iteration numbers for user-friendly display
        # (1st completed trial, 2nd completed trial, etc. instead of 0-indexed)
        df = pd.DataFrame(
            {
                "iteration_number": list(range(1, len(trace) + 1)),
                "utility": trace,
            }
        )
        ...
        if experiment.is_moo_problem:
            ...
            subtitle = (
                ...
                f"completed trials. {_ITERATION_NUMBER_EXPLANATION} The y-axis shows "
            ...
            if isinstance(objective, ScalarizedObjective):
                ...
                subtitle = (
                    ...
                    f"(objective is to {direction}). {_ITERATION_NUMBER_EXPLANATION} "
                ...
                subtitle = (
                    ...
                    f"{_ITERATION_NUMBER_EXPLANATION} {_CUMULATIVE_BEST_EXPLANATION} "
                    f"{_INFEASIBLE_TRIALS_EXPLANATION}"
                )
        ...
        fig.update_xaxes(
            title_text="Iteration Number", dtick=1, rangemode="nonnegative"
        )
