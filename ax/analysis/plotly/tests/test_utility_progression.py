...
class TestUtilityProgressionAnalysis(TestCase):
    def test_utility_progression_soo(self) -> None:
        ...
        self.assertIn("trial_number", card.df.columns)
    ...
    def test_scalarized_objective_support(self) -> None:
        ...
        # Assert: Check that title/subtitle show the formula with weights
        # ScalarizedObjective uses metrics m1 (weight=1.0) and m3 (weight=2.0)
        self.assertIn("Scalarized Objective", card.title)
        self.assertIn("m1 + 2.0*m3", card.title)
