...
class TestOptimizationTrace(TestCase):
    ...
    def setUp(self):
        ...
        self.metric = self.experiment.optimization_config.objective.metric
        ...
        for minimize in [True, False]:
            ...
            for data in fig.data:
                if len(data.y) == 1 and data.y[0] is None:
                    ...
                    continue
                elif data.name == "Sobol":
                    self.assertEqual(len(data.y), 2)
                elif data.name == "BoTorch":
                    self.assertEqual(len(data.y), 2)
                elif data.name == "Running optimum":
                    # Verify running optimum is continually improving
                    optimization_mult = -1 if minimize else 1
                    self.assertListEqual(
                        sorted(optimization_mult * data.y),
                        list(optimization_mult * data.y),
                    )
                else:
                    raise self.fail(msg="Found unaccounted for data object in test")

    def test_optimization_trace_errors(self):
