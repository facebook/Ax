...
def compute_cross_validation_adhoc(
    ...
    Args:
        ...
        generation_strategy: Optional. The generation strategy to extract the adapter
            from.
        adapter: Optional. The adapter to cross validate.
        data: Optional. The data to cross validate. If present the adapter will fit
            using this data.
    """
    relevant_adapter = extract_relevant_adapter(
        experiment=experiment,
    ...
    # If an Experiment and Data are provided, fit the adapter with the Data
    if experiment is not None and data is not None:
        observations, search_space = relevant_adapter._process_and_transform_data(
            experiment=experiment, data=data
        )
        relevant_adapter._fit_if_implemented(
            search_space=search_space, observations=observations, time_so_far=0
        )
