...
from ax.analysis.healthcheck.early_stopping import EarlyStoppingAnalysis
...
class TestEarlyStoppingAnalysis(TestCase):
    def test_early_stopping_with_savings(self) -> None:
        ...
        # Attach map data for both trials - trial 0 was already attached by
        # get_experiment_with_map_data()
        # but we need data for trial 1 as well to calculate savings
    ...
    def test_warnings(self) -> None:
        ...
        for name, experiment, strategy, attach_data, check_card in test_cases:
            with self.subTest(name=name):
                ...
                healthcheck = EarlyStoppingAnalysis(early_stopping_strategy=strategy)
                card = healthcheck.compute(experiment=experiment)
                check_card(card)

    def test_validate_applicable_state_no_experiment(self) -> None:
        """Test that validate_applicable_state returns an error message when
        experiment is None."""
        healthcheck = EarlyStoppingAnalysis()
    ...
    def test_fail_no_metrics_found(self) -> None:
        ...
        healthcheck = EarlyStoppingAnalysis(early_stopping_strategy=NoMetricStrategy())
        card = healthcheck.compute(experiment=experiment)
