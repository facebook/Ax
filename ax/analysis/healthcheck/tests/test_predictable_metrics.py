...
class TestPredictableMetricsAnalysis(TestCase):
    def test_passes_when_all_metrics_predictable(self) -> None:
        # GIVEN an experiment and generation strategy with good model fit
        ...
        with patch(
            "ax.service.utils.report_utils.compute_model_fit_metrics_from_adapter"
        ) as mock_compute:
    ...
    def test_warns_when_metrics_unpredictable(self) -> None:
        # GIVEN an experiment and generation strategy with poor model fit
        experiment = get_branin_experiment(with_completed_trial=True)
        mock_gs = MagicMock()
        mock_adapter = MagicMock()
        mock_gs.adapter = mock_adapter
        ...
        with patch(
            ...
            # THEN it returns WARNING status
            self.assertEqual(card.name, "PredictableMetricsAnalysis")
    ...
    def test_random_adapter_not_applicable(self) -> None:
        ...
        # WHEN validate_applicable_state is called
        healthcheck = PredictableMetricsAnalysis()
        ...
        # THEN it returns an error message about RandomAdapter
    ...
    def test_validate_applicable_state(self) -> None:
        ...
        mock_gs = MagicMock()
        mock_gs.adapter = MagicMock()  # Non-RandomAdapter
        ...
        with self.subTest("random_adapter"):
            # WHEN adapter is RandomAdapter
            # THEN validation fails (temporary state - GS will progress)
            mock_gs_random = MagicMock()
            mock_gs_random.adapter = RandomAdapter(
                experiment=experiment,
                generator=RandomGenerator(),
            )
            error = healthcheck.validate_applicable_state(
                experiment=experiment, generation_strategy=mock_gs_random
            )
            self.assertIsNotNone(error)
            self.assertIn("RandomAdapter", error)
