...
class MetricFetchingErrorsAnalysis(HealthcheckAnalysis):
    """
    Analysis for if any metric fetch errors have occurred for the experiment.
    ...
    def compute(
        ...
        metric_fetch_errors = [
            e for e in metric_fetch_errors if self._validate_fields(e=e)
        ]
        ...
        subtitle = df.copy()
        subtitle = subtitle.rename(columns=subtitle_df_columns)
    ...
    @staticmethod
    def _validate_fields(e: dict[str, Union[int, str]]) -> bool:
        required_fields = {
            "trial_index",
            "metric_name",
            "reason",
        ...
        if not required_fields.issubset(e.keys()):
            missing_fields = required_fields - e.keys()
            logger.warning(
                f"Metric fetch error {e} is missing required fields "
        ...
        if not set(e.keys()).issubset(required_fields.union(optional_fields)):
            extra_fields = set(e.keys()) - required_fields.union(optional_fields)
            logger.warning(
                f"Metric fetch error {e} has extra fields {extra_fields}.  Discarding."
            )
            return False
