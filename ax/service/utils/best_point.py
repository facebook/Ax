...
def _compute_utility_from_preference_model(
    ...
    try:
        adapter = TorchAdapter(
            ...
            search_space=pe_experiment.search_space,
            data=pe_data,
    ...
    # Get the fitted PairwiseGP model from the adapter
    generator = assert_is_instance(adapter.generator, BoTorchGenerator)
    pairwise_model = generator.surrogate.model

    # Create tensor of metric values for prediction
    # Shape: (n_arms, n_metrics)
    metric_values_list = []
    for _, row in df_wide.iterrows():
        metric_vals = [row[metric_name] for metric_name in metric_names]
        metric_values_list.append(metric_vals)

    X = torch.tensor(metric_values_list, dtype=torch.double)

    # Predict utilities using the preference model
    try:
        with torch.no_grad():
            posterior = pairwise_model.posterior(X)
            # pyre-ignore[16]: Posterior from PairwiseGP has mean property
            utilities = posterior.mean.squeeze(-1).numpy()
    except Exception as e:
        raise DataRequiredError(
    ...
    # Set infeasible arms to negative infinity
    is_feasible = df_wide["feasible"].to_numpy()
    utilities[~is_feasible] = float("-inf")
...
def get_trace(
    ...
    For preference learning experiments, the utility of each trial is computed using
