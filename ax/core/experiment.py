...
class Experiment(Base):
    ...
    def add_parameters_to_search_space(
        ...
        # If a trial exists, check that backfill values are provided
        if len(self.trials) != 0 and any(
            parameter.backfill_value is None for parameter in parameters
        ):
    ...
    def _get_last_data_without_similar_rows(
        ...
        merge_keys = ["trial_index", "metric_name", "arm_name"] + (
            cast(MapData, last_data).map_keys
