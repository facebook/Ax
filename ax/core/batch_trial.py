...
class BatchTrial(BaseTrial):
    ...
    *deployed and evaluated together*, and possibly arm weights, which are
    ...
    def add_arm(
        ...
        if generator_run_type == GeneratorRunType.STATUS_QUO:
            if proposed_name is None:
                raise UserInputError("Expected a proposed name for the status quo arm.")
    ...
    @immutable_once_run
    def add_status_quo_arm(self, status_quo: Arm, weight: float = 1.0) -> BatchTrial:
        ...
        for existing_arm in self.arm_weights:
            if (
                existing_arm.signature == status_quo.signature
                and float(weight) != self.arm_weights[existing_arm]
            ):
                raise UnsupportedError(
                    f"Status quo arm {existing_arm.name} is already added to the "
                    f"trial with weight {self.arm_weights[existing_arm]}. Reassigning the "
                    "weight is no longer supported."
                )
        if any(
            gr._generator_run_type == GeneratorRunType.STATUS_QUO
            for gr in self.generator_runs
        ):
            if (sq := self.status_quo) is None:
                raise AxError(
    ...
    @immutable_once_run
    def remove_status_quo_arm(self) -> BatchTrial:
        """Sets status quo arm with given weight. This weight *overrides* any
        weight the status quo has from generator runs attached to this batch.
        Thus, this function is not the same as using add_arm, which will
        result in the weight being additive over all generator runs.
        ...
        raise AxError("Statuos quo arm not found on trial #{self.index}.")
    ...
    def clone_to(
        ...
        new_trial = experiment.new_batch_trial(
            ...
            should_add_status_quo_arm=include_sq and self._status_quo is not None,
        )
        for gr in self.generator_runs:
            if gr._generator_run_type == GeneratorRunType.STATUS_QUO.name:
                # We don't need to add tis GR because `should_add_status_quo_arm`
                # argument to the trial constructor takes case of adding it.
                # TODO: In the future, we should eliminate `_status_quo(_override)`
                # attributes or the SQ GR, then we can just clone the GRs.
                continue
            # don't clone gen run in case we are attaching cloned trial to
            # the same experiment
            new_trial.add_generator_run(gr if use_old_experiment else gr.clone())
