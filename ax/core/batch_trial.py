...
class BatchTrial(BaseTrial):
    ...
    *deployed and evaluated together*, and possibly arm weights, which are
    ...
    Args:
        ...
        should_add_status_quo_arm: If True and the status quo arm is not alraedy
    ...
    def add_status_quo_arm(self, weight: float = 1.0) -> BatchTrial:
        ...
        if any(
            ...
            raise UnsupportedError(
                f"Trial {self.index} already has a status quo arm: {status_quo.name}."
            )
    ...
    def remove_status_quo_arm(self) -> BatchTrial:
        ...
        raise AxError("Status quo arm not found on trial #{self.index}.")
