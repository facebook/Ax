...
class BatchTrialTest(TestCase):
    ...
    def test_TTL_status_property_check(self) -> None:
        ...
        self.batch.mark_running(no_runner_required=True)
        older_candidate = self.experiment.new_batch_trial(ttl_seconds=1)
        sleep(0.1)  # Small delay to ensure different creation times
        latest_candidate = self.experiment.new_batch_trial(ttl_seconds=1)
        self.assertTrue(self.batch.status.is_running)
        self.assertTrue(older_candidate.status.is_candidate)
        self.assertTrue(latest_candidate.status.is_candidate)
        sleep(1)  # Wait 1 second for trial TTL to elapse.
        self.assertTrue(self.batch.status.is_running)
        # Older candidate should become stale, latest should remain candidate
        self.assertTrue(older_candidate.status.is_stale)
        self.assertTrue(latest_candidate.status.is_candidate)
        self.assertIn(0, self.experiment.trial_indices_by_status[TrialStatus.RUNNING])
        self.assertIn(1, self.experiment.trial_indices_by_status[TrialStatus.STALE])
        self.assertIn(2, self.experiment.trial_indices_by_status[TrialStatus.CANDIDATE])
    ...
    def test_TTL_experiment_trials_check(self) -> None:
        ...
        self.assertEqual(self.batch._status, TrialStatus.RUNNING)
        self.assertEqual(older_candidate._status, TrialStatus.STALE)
        self.assertEqual(latest_candidate._status, TrialStatus.CANDIDATE)
        self.assertIn(0, self.experiment.trial_indices_by_status[TrialStatus.RUNNING])
        self.assertIn(1, self.experiment.trial_indices_by_status[TrialStatus.STALE])
        self.assertIn(2, self.experiment.trial_indices_by_status[TrialStatus.CANDIDATE])
