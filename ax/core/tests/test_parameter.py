...
class DerivedParameterTest(TestCase):
    def setUp(self) -> None:
        ...
        self.param1 = DerivedParameter(
            name="x",
            parameter_type=ParameterType.FLOAT,
            parameter_names_to_weights={"a": 2.0},
            intercept=1.0,
        )
        ...
        self.param2 = DerivedParameter(
            name="x2",
            parameter_type=ParameterType.INT,
            parameter_names_to_weights={"a": 2.0, "b": 3.0},
        )
    ...
    def test_set_parameter_names_to_weights(self) -> None:
        new_parameter_names_to_weights = {"c": 5.0}
        self.param1.set_parameter_names_to_weights(
            parameter_names_to_weights=new_parameter_names_to_weights
        )
        self.assertEqual(
            self.param1.parameter_names_to_weights, new_parameter_names_to_weights
        )
        with self.assertRaisesRegex(
            UserInputError,
            "Derived parameters must have at least one parameter in "
            ...
            self.param1.set_parameter_names_to_weights(parameter_names_to_weights={})
