...
def _filter_data_on_status(
    ...
    dfs = []
    metric_sig_to_name = {m.signature: m.name for m in experiment.metrics.values()}
    for g, d in df.groupby(by="metric_signature"):
        metric_signature = g
        if metric_signature not in metric_sig_to_name.keys():
            ...
            continue
        metric = experiment.metrics[metric_sig_to_name[metric_signature]]
