...
class DummyData(AbstractDataFrameData):
    ...
    def __init__(
        ...
        # Represent DummyData internally as a flat `DataFrame`
    ...
    def from_multiple_data(
        data: Iterable[DummyData], subset_metrics: Optional[Iterable[str]] = None
    ) -> DummyData:
        ...
        subclass of `DummyData`, object of that class will be returned. If
        the iterable contains multiple types of `Data`, an error will be
        raised.

        Args:
            data: Iterable of Ax `DummyData` objects to combine.
            subset_metrics: If specified, combined `DummyData` will only contain
        ...
        # cast to list
        data = list(data)
        if not all((type(datum) is DummyData) for datum in data):
            # check if all types in iterable match the first type
            raise ValueError("Non-DummyData in inputs.")
        ...
        if not all((set(datum.map_keys) == set(map_keys)) for datum in data):
            ...
            return DummyData(df=pd.concat(dfs, axis=0, sort=True), map_keys=map_keys)
    ...
    def update(self, new_data: DummyData) -> None:
    ...
    def from_map_evaluations(
        ...
        """
        Convert dict of mapped evaluations to an Ax DummyData object
        ...
        return DummyData(df=pd.DataFrame(records), map_keys=map_keys)
