...
class TestPercentileEarlyStoppingStrategy(TestCase):
    ...
    def test_patience_zero_uses_single_point_evaluation(self) -> None:
        """Test that patience=0 (default) evaluates only the latest step."""
        exp = get_test_map_data_experiment(
            num_trials=5,
            num_fetches=3,
            num_complete=4,
        ...
        based on its performance at step==2.
        """
        early_stopping_strategy = PercentileEarlyStoppingStrategy(
            percentile_threshold=25,
            min_progression=0,
    ...
    def test_patience_with_consistent_underperformance(self) -> None:
        """Test that trials are stopped when they consistently underperform
        across the entire patience window."""
        exp = get_test_map_data_experiment(
            num_trials=5,
            num_fetches=3,
            num_complete=4,
    ...
    def test_patience_with_inconsistent_underperformance(self) -> None:
        """Test that trials are NOT stopped when they don't consistently
        underperform across the patience window."""
    ...
    def test_patience_with_insufficient_progressions(self) -> None:
        """Test that trials are not stopped when there are insufficient
        progressions in the patience window."""
        exp = get_test_map_data_experiment(
            num_trials=5,
        ...
        With only 1 progression in the window, the condition
        `len(window_progressions) > 1` should be False.
        """
        # Use patience=2 which requires multiple progressions in window
        early_stopping_strategy = PercentileEarlyStoppingStrategy(
            percentile_threshold=25,
            min_curves=3,
    ...
    def test_patience_with_insufficient_curves_at_progression(self) -> None:
        """Test that trials are not stopped when there are insufficient
        curves/trials with data at one or more progressions in the patience window."""
        exp = get_test_map_data_experiment(
            num_trials=5,
            num_fetches=3,
            num_complete=5,
        ...
        # Now modify the data to simulate insufficient trials at progression 1
        # Remove data for trials 2, 3, 4 at progression 1
        ...
        step        0         1         2        3         4
        ------  -------  --------  --------  -------  --------
            0  146.139  113.057    44.6272  143.376   65.0335
            1  117.388   90.8152   35.8475  115.169   52.2392
            2   99.95    77.3245  nan       nan      nan

        - At progression 0: all 5 trials have data
        - At progression 1: all 5 trials have data
        - At progression 2: only trials 0, 1 have data (2 < min_curves=4)

        When evaluating trial 0 at progression 2 with patience=1 (window [1, 2]):
        - At progression 1: all 5 trials have data (>= min_curves=4)
        - At progression 2: only 2 trials have data (< min_curves=4)

        Trial should NOT be stopped due to insufficient curves at progression 2.
        """
        ...
        # Should not stop due to insufficient trials at progression 1
        self.assertFalse(should_stop)
        reason = none_throws(reason)
        self.assertRegex(
            reason,
        ...
        self.assertRegex(reason, r"Minimum required: 4")
    ...
    def test_patience_with_large_window(self) -> None:
        """Test patience with a large window that spans all available progressions."""
        exp = get_test_map_data_experiment(
            num_trials=5,
            num_fetches=3,  # Creates progressions 0, 1, 2
            num_complete=4,
        )

        # Use patience=10 which is larger than the available progressions
        early_stopping_strategy = PercentileEarlyStoppingStrategy(
            percentile_threshold=25,
            min_progression=0,
            min_curves=4,
        ...
        # Should stop if trial 0 underperforms at all available steps
        self.assertTrue(should_stop)
        reason = none_throws(reason)
        # Window should span all available progressions
        self.assertRegex(reason, r"Trial objective values at progressions")

    def test_patience_protects_trials_with_noisy_curves(self) -> None:
    ...
    def test_patience_respects_min_progression(self) -> None:
        ...
        This ensures we only evaluate based on data from progressions >= min_progression.
        """
        early_stopping_strategy = PercentileEarlyStoppingStrategy(
            percentile_threshold=50,
            min_curves=4,
        ...
        self.assertRegex(
            reason,
            r"Trial objective values at progressions in \[2\.0, 4\.0\]",
        )
