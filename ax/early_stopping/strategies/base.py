...
class BaseEarlyStoppingStrategy(ABC, Base):
    ...
    def estimate_early_stopping_savings(
        ...
        if map_key is None:
            if self.metric_signatures:
                first_metric_signature = next(iter(self.metric_signatures))
                first_metric = experiment.signature_to_metric[first_metric_signature]
    ...
    def is_eligible(
        ...
        Args:
            ...
            df: A dataframe containing the time-dependent metrics for the trial.
                NOTE: `df` should only contain data with `metric_signature` fields that are
    ...
    def _all_objectives_and_directions(self, experiment: Experiment) -> dict[str, bool]:
        """Returns a dict containing the metric signatures and corresponding directions for
        each objective in the experiment or in `self.metric_signatures`, if specified.
        """
        if self.metric_signatures is None:
            logger.debug(
                "No metric signatures specified. Defaulting to the objective metric(s).",
                stacklevel=2,
            )
