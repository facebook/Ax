...
FASHION_MNIST_SURROGATE_OPTIMAL_VALUE = 0.9225427
...
def get_fashion_mnist_surrogate_benchmark(
    num_trials: int = 50,
) -> BenchmarkProblem:
    """Create a BenchmarkProblem for tuning hyperparameters of a residual network on
    FashionMNIST. The tunable hyperparameters are as follows:
    1. The learning rate;
    ...
    `get_fashion_mnist_surrogate_search_space`. The returned benchmark problem uses
    gradient-boosted trees to predict the validation accuracy, which makes the oracle
    computationally cheap to evaluate.
    """
    search_space = get_fashion_mnist_surrogate_search_space()
    ...
    test_function = HierarchicalSearchSpaceSurrogate(
        outcome_names=["FashionMNIST Test Accuracy"],
        lst_active_param_names=lst_active_param_names,
        flag_param_names=flag_param_names,
        lst_flag_config=lst_flag_config,
        lst_xgb_models=lst_xgb_models,
    ...
    return BenchmarkProblem(
        ...
        optimal_value=FASHION_MNIST_SURROGATE_OPTIMAL_VALUE,
        test_function=test_function,
        baseline_value=FASHION_MNIST_SURROGATE_BASELINE_VALUE,
    )
