...
class TestAxOrchestrator(TestCase):
    ...
    def test_run_trials_and_yield_results_with_early_stopper(self) -> None:
        ...
        with (
            ...
            # Extract max_parallelism from generation_blocking_criteria
            node0_max_parallelism = None
            for bc in self.two_sobol_steps_GS._nodes[0].generation_blocking_criteria:
                if isinstance(bc, MaxGenerationParallelism):
                    node0_max_parallelism = bc.threshold
                    break
            self.assertEqual(
